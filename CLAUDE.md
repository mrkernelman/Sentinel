...
- Docker Hub images (`docker-compose.hub.yml`, still `jeffreyjr/*`) are pre-built with the old absolute `NEXT_PUBLIC_API_URL` and are **not** affected by the `lib/api.ts` fix on their own.
- **`docker-compose.hub-nginx.yml`** (standalone — `docker compose -f docker-compose.hub-nginx.yml up -d`, dashboard on `:8080`) is the portable Hub deployment and needs **no local build at all**: `db` + `backend` (`jeffreyjr/shadow-it-backend`, baked-in ML artifacts) + `frontend` (**`radio123/shadow-it-frontend`**, a rebuild of the same source with `NEXT_PUBLIC_API_URL=""`) are all pre-built images, fronted by the same nginx. First `up -d` on a clean host takes ~20s (image pulls only, no npm build) vs. several minutes when the frontend was built locally. Deliberately *not* an overlay on `docker-compose.hub.yml` — it duplicates the `db`/`backend` service definitions so it runs standalone with one `-f` flag.
