...
- Docker Hub images (`docker-compose.hub.yml`) are pre-built with the old absolute `NEXT_PUBLIC_API_URL` and are **not** affected by the `lib/api.ts` fix on their own — they'd need a rebuild-and-repush with `NEXT_PUBLIC_API_URL=""` to benefit (not done; ask before touching published Hub images).
- **`docker-compose.hub-nginx.yml`** (standalone — `docker compose -f docker-compose.hub-nginx.yml up -d --build`, dashboard on `:8080`) is the fix for the Hub deployment: keeps the published `jeffreyjr/shadow-it-backend` image as-is (baked-in ML artifacts, no local `ml/artifacts/`/`data/` needed) but builds the **frontend** locally with `NEXT_PUBLIC_API_URL=""`, fronted by the same nginx. Deliberately *not* an overlay on `docker-compose.hub.yml` — it duplicates the `db`/`backend` service definitions so it runs standalone with one `-f` flag and doesn't inherit `docker-compose.hub.yml`'s baked-URL frontend or its published 5000/3005 ports.
